...
async function main() {
  ...
  try {
    ...
    if (t.includes('App')) {
      ...
      console.log('  18.02.2023 dan beri third-party SMS ololmaydi — faqat app. Agar hech qaerda sessiyangiz yo\'q bo\'lsa ham shunday bo\'ladi.');
    ...
    else if (t.includes('Sms')) console.log('→ Code sent via SMS (check SMS inbox, wait 60-120s, Firebase SMS faqat official app da)');
    ...
    else if (t.includes('FlashCall')) console.log('→ FlashCall — pattern:', (sent.type as unknown as { pattern?: string })?.pattern);
    else if (t.includes('SetUpEmailRequired')) console.log('→ Telegram login EMAIL so\'radi — avval email o\'rnating (official app da)');
    else console.log('→ Unknown type, check full above — likely App');

    if (sent.nextType) console.log(`Next fallback (after timeout): ${sent.nextType.className} — use auth.ResendCode with phoneCodeHash`);
    if (sent.timeout) console.log(`Resend allowed after ${sent.timeout}s — wait then try resend`);
    else console.log('Note: timeout=null → code often never arrives (flagged IP or 2025-2026 Telegram bug) — try QR: npm run login:qr');
      ...
      if (msg.includes('SESSION_PASSWORD_NEEDED')) {
        ...
        try {
          ...
          if (m2.includes('EMAIL_UNCONFIRMED') || m2.includes('EMAIL')) {
            ...
            const emailCode = await ask('Email code (pochtadan): ');
            // @ts-ignore - teleproto may require emailVerification param
            // Try next step if needed; fallback is manual via official app
            console.error('Email after 2FA not fully automated in this script — please use official app to confirm email, then retry QR');
            throw e2;
          } else throw e2;
        }
        ...
        if (rr.toLowerCase().startsWith('y')) {
          try {
            const resent2 = (await client.invoke(new Api.auth.ResendCode({ phoneNumber: phone, phoneCodeHash: sent.phoneCodeHash }))) as unknown as typeof sent;
    ...
    if (m.includes('SEND_CODE_UNAVAILABLE')) console.error('→ Barcha kod yuborish usullari tugadi — QR ishlating: npm run login:qr');
  } finally {
