...
export async function joinApproved(partnerId: number, deal: DealLike, role: 'sotuvchi' | 'xaridor'): Promise<void> {
  const text = [`✅ Deal #${deal.id} ga qo'shildingiz (${role}).`, `${deal.amount} ${deal.asset} — keyingi qadam ilovada.`, `Ilovani oching.`].join('\n');
...
export async function depositToSeller(sellerId: number, deal: DealLike): Promise<void> {
  const text = [`💰 Pul keldi: ${deal.amount} ${deal.asset} (Deal #${deal.id}).`, `Iltimos, tovarni xaridorga bering.`, `Keyin ilovada "Yubordim" ni bosing.`].join('\n');
  await send(sellerId, text, deal.id);
}

export async function shippedToBuyer(buyerId: number, deal: DealLike): Promise<void> {
  const text = [`📦 Sotuvchi yetkazdi (Deal #${deal.id}).`, `Oldingizmi? Iltimos, ilovada tasdiqlang.`, `Ilovani oching.`].join('\n');
  await send(buyerId, text, deal.id);
}

export async function releasedToBuyer(buyerId: number, deal: DealLike): Promise<void> {
  const text = [`✅ Deal #${deal.id} yakunlandi.`, `${deal.amount} ${deal.asset} sotuvchiga o'tdi.`, `Xizmatdan yana foydalaning.`].join('\n');
  await send(buyerId, text, deal.id);
}

export async function releasedToSeller(sellerId: number, deal: DealLike, netAmount: string): Promise<void> {
  const text = [`🎉 Deal #${deal.id}: ${netAmount} ${deal.asset} oldingiz.`, `Komissiya chegirildi.`, `Ilovada tekshiring.`].join('\n');
  await send(sellerId, text, deal.id);
}
...
export async function reminderToPayer(buyerId: number, deal: DealLike): Promise<void> {
  const text = [`⏰ Eslatma: Deal #${deal.id} uchun ${deal.amount} ${deal.asset} to'lang.`, `Vaqt tugamasdan ilovada to'lov qiling.`].join('\n');
  await send(buyerId, text, deal.id);
}
...
export async function unknownDepositToAdmins(info: { amount: string | number; asset: string; address: string; memo: string }): Promise<void> {
  try {
    const bot = getBot();
    if (!bot) return;
    const text = [`⚠️ Noma'lum to'lov: ${info.amount} ${info.asset}`, `Manzil: ${info.address}`, `Memo: ${info.memo}`].join('\n');
