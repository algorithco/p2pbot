  ...
  var TG = {
    ...
    init: function () {
      ...
      var applyVh = function () {
        var h = wa.viewportStableHeight && wa.viewportStableHeight > 0 ? wa.viewportStableHeight : window.innerHeight;
        document.documentElement.style.setProperty('--app-height', h + 'px');
      };
      applyVh();
      safe(function () {
        wa.onEvent('viewportChanged', applyVh);
      });
      ...
      applyScheme();
      safe(function () {
        wa.onEvent('themeChanged', applyScheme);
      });
    },
    ...
    main: {
      ...
      hideProgress: function () {
        safe(function () {
          if (wa && wa.MainButton) wa.MainButton.hideProgress();
        });
      },
      hide: function () {
        safe(function () {
    ...
    confirm: function (message, onYes) {
      safe(function () {
        if (isAtLeast(6.2) && wa.showConfirm) {
          wa.showConfirm(String(message), function (ok) {
            if (ok && onYes) onYes();
          });
          return;
        }
    ...
    preventClose: function (on) {
      safe(function () {
        if (isAtLeast(7.0) && wa.enableClosingConfirmation) {
